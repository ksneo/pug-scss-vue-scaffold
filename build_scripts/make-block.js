        ...
        fs.readFile(importFile, 'utf8', (err, data) => {
            ...
            if (err) {
                if (err.code === 'ENOENT') {
                    ...
                    reject(
                        new Error(
                            `ERR>>> Failed to read a import file '${importFile}'`
                        )
                    );
                }
            } else {
                ...
                if (alreadyHasImport) {
                    reject(new Error(`Block ${blockName} is already imported`));
                } else {
                    resolve(data + res);
                }
            }
...
function createImportFile(filePath, blockData) {
    return new Promise((resolve, reject) => {
        fs.writeFile(filePath, blockData, 'utf8', err => {
            if (err) {
                reject(
                    new Error(`ERR>>> Failed to create a file '${filePath}'`)
                );
            } else {
                resolve();
            }
    ...
    return new Promise((resolve, reject) => {
        fs.stat(blockPath, notExist => {
            if (notExist) {
                ...
                reject(
                    new Error(`ERR>>> The block '${blockName}' already exists.`)
                );
            }
        });
    });
...
function createDir(dirPath) {
    return new Promise((resolve, reject) => {
        fs.mkdir(dirPath, err => {
            if (err) {
                reject(
                    new Error(`ERR>>> Failed to create a folder '${dirPath}'`)
                );
            } else {
                resolve();
            }
            ...
            new Promise((resolve, reject) => {
                fs.writeFile(filePath, fileSource, 'utf8', err => {
                    if (err) {
                        reject(
                            new Error(
                                `ERR>>> Failed to create a file '${filePath}'`
                            )
                        );
                    } else {
                        resolve();
...
function initMakeBlock(blockName, pageName) {
    ...
    return validateBlockName(blockName)
        ...
        .then(files => {
            ...
            console.log(line);
            console.log(
                `The block has just been created in '${BLOCKS_DIR}/${blockName}'`
            );
            console.log(line);
